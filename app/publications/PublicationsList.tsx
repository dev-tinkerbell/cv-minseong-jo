...
function highlightAuthor(author: string) {
  ...
  return (
    <span
      key={author}
      style={{ color: isMe ? "#c8d8ff" : "#6b7280", fontWeight: isMe ? 600 : 400 }}
    >
      {author}
    </span>
  );
...
function PublicationCard({ pub, index }: { pub: Publication; index: number }) {
  return (
    <motion.article
      ...
      className="p-5 md:p-6 rounded"
      style={{ backgroundColor: "#111118", border: "1px solid #1e1e2e" }}
    >
      <div className="flex items-start justify-between gap-3 mb-3">
        <div className="flex items-center gap-2 flex-wrap">
          <span className="font-mono text-xs" style={{ color: "#9ba3b2" }}>
            {pub.journal}
          </span>
      ...
      <p className="text-xs leading-relaxed mb-4" style={{ color: "#6b7280", lineHeight: 1.7 }}>
        {pub.authors.map((a, i) => (
          <span key={i}>
            {highlightAuthor(a)}
      ...
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div className="flex flex-wrap gap-1.5">
          {pub.tags.map((tag, i) => (
            <span
              ...
              style={{ color: "#9ba3b2", borderColor: "#1e1e2e", backgroundColor: "#0d0d14" }}
            >
              {tag.label}
            </span>
          ))}
        ...
        {pub.doi ? (
          ...
          <span className="text-xs font-mono shrink-0" style={{ color: "#2a2a3a" }}>
            DOI pending
          </span>
        )}
      </div>
        ...
        <div className="flex gap-2 min-w-max md:min-w-0 md:flex-wrap">
          {filters.map((f) => (
            <button
              ...
              style={{
                backgroundColor: activeFilter === f.id ? "#4a7fff" : "#111118",
                color: activeFilter === f.id ? "#f0f4ff" : "#6b7280",
                border: activeFilter === f.id ? "1px solid #4a7fff" : "1px solid #1e1e2e",
              }}
            >
              {f.label}
            </button>
